...
```bash
npx @creative-tim/npm-xyz@latest components add button
```
...
You can install Creative Tim UI components using either the `@creative-tim/npm-xyz` CLI or the `shadcn/ui` CLI. Both achieve the same result: adding the selected component's code and any needed dependencies to your project.
